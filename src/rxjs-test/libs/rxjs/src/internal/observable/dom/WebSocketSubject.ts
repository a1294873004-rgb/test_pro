import type {
  Subscriber,
  Subscription,
} from "src/rxjs-test/libs/observable/src";
import { Observable, operate } from "src/rxjs-test/libs/observable/src";
import type { NextObserver } from "../../types";
...
export class WebSocketSubject<In, Out = In> extends Observable<Out> {
  private _config: WebSocketSubjectConfig<In, Out> &
    Required<
      Pick<
        WebSocketSubjectConfig<In, Out>,
        "WebSocketCtor" | "serializer" | "deserializer"
      >
    >;
  ...
  constructor(urlConfigOrSource: string | WebSocketSubjectConfig<In, Out>) {
    super();
    const userConfig =
      typeof urlConfigOrSource === "string"
  ...
  multiplex(
    subMsg: () => In,
    unsubMsg: () => In,
    messageFilter: (value: Out) => boolean
  ) {
